...
(define cps-out-grammar
  '([cps-out-program (tfexp) cps-a-program]
    ...
    [simple-expression ("emptylist") cps-emptylist-exp]
    [simple-expression ("list" "(" (separated-list simple-expression ",") ")") cps-list-exp]
    [simple-expression ("car" "(" simple-expression ")") cps-car-exp]
    [simple-expression ("cdr" "(" simple-expression ")") cps-cdr-exp]
    [simple-expression ("cons" "(" simple-expression "," simple-expression ")") cps-cons-exp]
...
(define inp-exp-simple?
  (lambda (exp)
    (cases expression exp
      ...
      [emptylist-exp () #t]
      [list-exp (exps) (all-simple? exps)]
      [car-exp (exp1) (inp-exp-simple? exp1)]
      [cdr-exp (exp1) (inp-exp-simple? exp1)]
      [cons-exp (exp1 exp2) (all-simple? (list exp1 exp2))]
      [else #f])))
...
(define cps-of-let-exp
  (lambda (ids rhss body k-exp)
    (cps-of-exps rhss
                 (lambda (new-rands)
                   (cps-let-exp ids new-rands (cps-of-exp body k-exp))))))
...
(define cps-of-simple-exp
  (lambda (exp)
    (cases expression exp
      ...
      [cons-exp (exp1 exp2) (cps-cons-exp (cps-of-simple-exp exp1)
...
(define cps-of-exp
  (lambda (exp cont)
    (cases expression exp
      ...
      [emptylist-exp () (cps-of-emptylist-exp cont)]
      [list-exp (exps) (cps-of-list-exp exps cont)]
      [car-exp (exp1) (cps-of-car-exp exp1 cont)]
      [cdr-exp (exp1) (cps-of-cdr-exp exp1 cont)]
      [cons-exp (exp1 exp2) (cps-of-cons-exp exp1 exp2 cont)]
      ...
      [let-exp (vars exps body) (cps-of-let-exp vars exps body cont)]
...
(define value-of-simple-exp
  (lambda (exp env)
    (cases simple-expression exp
      ...
      [cps-emptylist-exp () (emptylist-val)]
      [cps-cons-exp (exp1 exp2) (let ([val1 (value-of-simple-exp exp1 env)]
                                      [val2 (value-of-simple-exp exp2 env)])
                                  (pair-val val1 val2))]
      [cps-car-exp (exp1) (let ([val1 (value-of-simple-exp exp1 env)])
                            (expval->car val1))]
      [cps-cdr-exp (exp1) (let ([val1 (value-of-simple-exp exp1 env)])
                            (expval->cdr val1))]
      [cps-list-exp (exps) (let ([vals (map (lambda (x) (value-of-simple-exp x env)) exps)])
                             (list-val vals))]
      )))
...
(define value-of/k
  (lambda (exp env cont)
    (cases tfexp exp
      ...
      [cps-let-exp (vars rhss body) (let ([vals (map (lambda (x) (value-of-simple-exp x env)) rhss)])
