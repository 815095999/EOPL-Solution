      ...
      (extend-env-rec** (p-names b-varss p-bodies saved-env)
                        (cond ((location search-sym p-names)
                               => (lambda (n) (newref (proc-val (procedure
                                                                 ...
                                                                 env)))))
                              (else (apply-env saved-env search-sym))))
      (extend-env-with-self-and-super (self super-name self-name saved-env)
                                      (case search-sym
                                        ((%self) self)
                                        ((%super) super-name)
                                        ((%self-name) self-name)
...
(define apply-method
  (lambda (m self args)
    (cases method m
      (a-method (vars body super-name self-name field-names)
                (value-of body
...
(define method-decls->method-env
  (lambda (m-decls super-name self-name field-names)
    (map
     (lambda (m-decl)
    ...
    (cases expression exp
      ...
      (method-call-exp (obj-exp method-name rands)
                       (let ((args (values-of-exps rands env))
                         ...
                         (apply-method
                          (if (self-exp? obj-exp)
                              (find-method (apply-env env '%self-name) method-name)
