     ...
     (lambda (m-decl)
       (cases method-decl m-decl
         (a-method-decl (method-name vars body)
                        (list (new-name method-name (length vars))
                              (a-method vars body super-name field-names)))))
     m-decls)))
    ...
    (cases expression exp
      ...
      (method-call-exp (obj-exp method-name rands)
                       (let ((args (values-of-exps rands env))
                         ...
                         (apply-method
                          (find-method (object->class-name obj) (new-name method-name (length args))) obj args)))
      (super-call-exp (method-name rands)
                      (let ((args (values-of-exps rands env))
                            (obj (apply-env env '%self)))
                        (apply-method
                         (find-method (apply-env env '%super) (new-name method-name (length args))) obj args)))
      (new-object-exp (class-name rands)
                      (let ([args (values-of-exps rands env)]
                            [obj (new-object class-name)])
                        (apply-method (find-method class-name (new-name 'initialize (length args))) obj args)
                        obj))
      )))
...
let obj = new a()
in list (send obj m1(), send obj m1(1, 2), send obj m1(3))
")
; #(struct:list-val (#(struct:num-val 0) #(struct:num-val -1) #(struct:num-val 3)))