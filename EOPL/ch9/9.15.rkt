...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (class-decl ("class" identifier "extends" identifier
                         (arbno "static" identifier "=" expression) (arbno "field" identifier) (arbno method-decl)) a-class-decl)
...
(define extend-env-with-static
  (lambda (obj env)
    (extend-env (object->static-vars obj)
                (object->static-fields obj)
                env)))
(define object->static-vars
  (lambda (obj)
    (class->static-vars (object->class-name obj))))
(define object->static-fields
  (lambda (obj)
    (class->static-fields (object->class-name obj))))
(define class->static-vars
  (lambda (c-name)
    (cases class (lookup-class c-name)
      (a-class (s-name s-vars s-fields fields-names menv)
               s-vars))))
...
(define-datatype class class?
  (a-class (super-name (maybe identifier?))
           (static-vars (list-of identifier?))
           (static-fields (list-of reference?))
           (field-names (list-of identifier?))
...
(define initialize-class-decl!
  (lambda (c-decl)
    (cases class-decl c-decl
      (a-class-decl (c-name s-name s-vars s-exps f-names m-decls)
                    (let ([f-names
                           (append-field-names
                            (class->field-names (lookup-class s-name))
                            f-names)]
                          [s-fields (map (lambda (exp) (newref (value-of exp (empty-env))))
                                         s-exps)])
                      (add-to-class-env!
                       c-name
                       (a-class s-name
...
(define class->super-name
  (lambda (c-struct)
    (cases class c-struct
	   (a-class (s-name s-vars s-fields field-names method-env)
		    s-name))))
...
(run "class queue extends object
      static counter = 0
      field q
      method empty() begin set counter = -(counter,-1); empty?(q) end
      method initialize() begin set q = list() end
