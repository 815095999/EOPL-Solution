...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (expression ("named-send" identifier expression identifier "(" (separated-list expression ",") ")") named-send-exp)
    ))
    ...
    (cases expression exp
      ...
      (new-object-exp (class-name rands)
                      (let ([args (values-of-exps rands env)]
                        ...
                        obj))
      (named-send-exp (class-name obj-exp method-name rands)
                      (let ((args (values-of-exps rands env))
                            (obj (value-of obj-exp env)))
...
(run "class oddeven extends object
      method initialize() 1
      method odd(x) if zero?(x) then 0 else named-send oddeven self even(-(x,1))
      method even(x) if zero?(x) then 1 else named-send oddeven self odd(-(x,1))
      class bogus_oddeven extends oddeven
      method odd(x) 2
      let x = new bogus_oddeven()
      in named-send oddeven x odd(11)")
