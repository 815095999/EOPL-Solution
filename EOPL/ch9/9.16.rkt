    ...
    (let ([m-env (class->method-env (lookup-class c-name))])
      (let loop ([menv m-env]
        ...
        (if (null? menv)
            ...
            (if (and (equal? (caar menv) name)
                     (equal? (method->len (cadar menv)) len))
                (cadar menv)
                (loop (cdr menv)
                      name len)))))))
(define method->len
  (lambda (meth)
    (cases method meth
      (a-method (vars body super-name fields-names)
                (length vars)))))
    ...
    (cases expression exp
      ...
      (method-call-exp (obj-exp method-name rands)
                       (let ((args (values-of-exps rands env))
                         ...
                         (apply-method
                          (find-method (object->class-name obj) method-name (length args)) obj args)))
      (super-call-exp (method-name rands)
                      (let ((args (values-of-exps rands env))
                            (obj (apply-env env '%self)))
                        (apply-method
                         (find-method (apply-env env '%super) method-name (length args)) obj args)))
