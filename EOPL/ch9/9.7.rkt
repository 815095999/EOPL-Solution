...
(define merge-method-envs
  (lambda (super-m-env new-m-env)
    (let loop([new-env new-m-env]
              [super-m-env super-m-env])
      (if (null? super-m-env)
          new-env
          (if (method-exist? (car super-m-env) new-env)
              (loop new-env
                    (cdr super-m-env))
              (loop (append new-env
                            (list (car super-m-env)))
                    (cdr super-m-env)))))))

; method-exist : method x method-env -> bool
(define method-exist?
  (lambda (meth menv)
    (if (null? menv)
        #f
        (if (equal? (method->name meth)
                    (method->name (car menv)))
            #t
            (method-exist? meth (cdr menv))))))
(define method->name
  (lambda (meth)
    (car meth)))
