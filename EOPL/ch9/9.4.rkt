...
(define-datatype object object?
  (an-object (class-type class?)
             (fields (list-of reference?))))

; new-object : sym -> obj
(define new-object
  (lambda (class-name)
    (an-object (lookup-class class-name)
...
(define find-method
  (lambda (c-type name)
    (let ([m-env (class->method-env c-type)])
    ...
    (cases expression exp
      ...
      (method-call-exp (obj-exp method-name rands)
                       (let ((args (values-of-exps rands env))
                         ...
                         (apply-method
                          (find-method (object->class-type obj) method-name) obj args)))
      ...
      (new-object-exp (class-name rands)
                      (let ([args (values-of-exps rands env)]
                        ...
                        (apply-method (find-method (lookup-class class-name) 'initialize) obj args)
                        obj))
      )))
...
(run "class oddeven extends object
      method initialize() 1
      method odd(x) if zero?(x) then 0 else send self even(-(x,1))
