...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (method-decl ("method" final-module identifier "("  (separated-list identifier ",") ")" 
                           expression) a-method-decl)
    (final-module ("final") a-final-module)
    (final-module () empty-final-module)
...
(define apply-method
  (lambda (m self args)
    (cases method m
      (a-method (is-final vars body super-name field-names)
                (value-of body
    ...
    (map
     (lambda (m-decl)
       (cases method-decl m-decl
         (a-method-decl (f-module method-name vars body)
                        (list method-name
                              (a-method (f-module->is-final f-module) vars body super-name field-names)))))
     m-decls)))

(define f-module->is-final
  (lambda (f-module)
    (cases final-module f-module
      (a-final-module () #t)
      (empty-final-module () #f))))
(define method->is-final
  (lambda (meth)
    (cases method meth
      (a-method (is-final vars body s-name f-names) is-final))))

; merge-method-envs : MethodEnv * MethodEnv -> MethodEnv
(define merge-method-envs
  (lambda (super-m-env new-m-env)
    (let loop ([super-m-env super-m-env]
               [new-m-env new-m-env])
      (if (null? new-m-env)
          super-m-env
          (if (method-exist-final? (car new-m-env) super-m-env)
              (eopl:error 'merge-method-envs "override final method! ~s" (car new-m-env))
              (cons (car new-m-env)
                    (loop super-m-env (cdr new-m-env))))))))
(define method-exist-final?
  (lambda (meth menv)
    (if (null? menv)
        #f
        (if (and (equal? (car meth) (car (car menv)))
                 (method->is-final (cadr (car menv))))
            #t
            (method-exist-final? meth (cdr menv))))))
...
(run "class oddeven extends object
      method initialize() 1
      method final odd(x) if zero?(x) then 0 else send self even(-(x,1))
      method even(x) if zero?(x) then 1 else send self odd(-(x,1))
      class bogus_oddeven extends oddeven
      method odd(x) 2
      let x = new bogus_oddeven()
      in send x odd(11)")
