...
(define the-grammar
  '((program ((arbno class-decl) expression) a-program)
    ...
    (class-decl ("interface" identifier
                 ...
                 (arbno abstract-method-decl)) an-interface-decl)
...
(define initialize-class-decl!
  (lambda (c-decl)
    (cases class-decl c-decl
      ...
      (an-interface-decl (i-name parents a-m-decls) #t))))
...
(define add-class-decl-to-static-class-env!
  (lambda (c-decl)
    (cases class-decl c-decl
      (an-interface-decl (i-name parents abs-m-decls)
                         (let ([local-method-tenv (abs-method-decls->method-tenv abs-m-decls)]
                               [parent-menvs (map (lambda (parent)
                                                    (cases static-class (lookup-static-class parent)
                                                      (an-interface (menv) menv)
                                                      (else (eopl:error 'add-class-decl-to-static-class-env! "~s 's parent not an interface ~s" i-name parent))))
                                                  parents)])
                           (let ([m-tenv (merge-method-tenvs
                                          (append-menvs parent-menvs)
                                          local-method-tenv)])
...
; append-menvs : listof(menv) -> tenv
(define append-menvs
  (lambda (menvs)
    (if (null? menvs)
        menvs
        (append (car menvs)
                (append-menvs (cdr menvs))))))
...
(define check-class-decl!
  (lambda (c-decl)
    (cases class-decl c-decl
      (an-interface-decl (i-name parents abs-method-decls) #t)
...
interface nothing
    method int sum ()
interface tree extends nothing
