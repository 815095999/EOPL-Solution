  ...
  (lambda (env search-sym)
    (cases environment env
      ...
      in begin send q1 enqueue(3);
               ...
               send q2 dequeue();
               send q2 empty();
               send q1 cnt() end")(eopl:error 'apply-env "No binding for ~s" search-sym))
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (superfieldref-exp (f-name)
                         (let ((the-ref (find-super-ref env f-name)))
                           (deref the-ref)))
      (superfieldset-exp (f-name exp)
                         (let ((the-ref (find-super-ref env f-name)))
                           (setref! the-ref (value-of exp env))))
      )))
...
(define find-super-ref
  (lambda (env f-name)
    (let* ((s-name (apply-env env '%super))
           (obj (apply-env env '%self))
           (fields (object->fields obj))
           (field-names (class->field-names (lookup-class s-name))))
      (apply-env (extend-env field-names fields (empty-env))
                 f-name))))
...
(run "class counter extends object
      ...
      class queue extends counter
      field q
      method empty() begin superfieldset val = -(superfieldref val,-1); empty?(q) end
      method initialize() begin super initialize(); set q = list() end
      method enqueue(x) begin set q = cons(x,q);
                              superfieldset val = -(superfieldref val,-1) end
      method dequeue() let a = car(q)
                       in begin set q = cdr(q);
                                superfieldset val = -(superfieldref val,-1);
                                a end
      method cnt() superfieldref val
      let q1 = new queue()
      in let q2 = new queue()
      in begin send q1 enqueue(3);
               send q1 dequeue();
