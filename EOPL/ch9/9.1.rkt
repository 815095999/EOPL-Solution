...
(define merge-method-envs
  (lambda (super-m-env new-m-env)
    (let loop ([super-m-env super-m-env]
      ...
      (if (null? new-m-env)
          super-m-env
          (loop (replace-or-expand super-m-env (car new-m-env))
                (cdr new-m-env))))))
(define replace-or-expand
  (lambda (super-m-env meth)
    (if (null? super-m-env)
        (list meth)
        (if (equal? (caar super-m-env) (car meth))
            (cons meth (cdr super-m-env))
            (cons (car super-m-env)
                  (replace-or-expand (cdr super-m-env) meth))))))
