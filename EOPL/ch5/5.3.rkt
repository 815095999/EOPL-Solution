...
(define-datatype continuation continuation?
  ...
  (rand-cont (rator expval?)
             (cont continuation?))
  (let2-after1-cont (var1 identifier?)
                    (var2 identifier?)
                    (exp2 expression?)
                    (body expression?)
                    (env environment?)
                    (cont continuation?))
  (let2-after2-cont (var1 identifier?)
                    (var2 identifier?)
                    (val1 expval?)
                    (body expression?)
                    (env environment?)
...
(define apply-cont
  (lambda (cont val)
    (cases continuation cont
      ...
      (let2-after1-cont (var1 var2 exp2 body env cont)
                        (value-of/k exp2 env (let2-after2-cont var1 var2 val body env cont)))
      (let2-after2-cont (var1 var2 val1 body env cont)
                        (value-of/k body (extend-env var1 val1 (extend-env var2 val env)) cont))
      )))
; ------------------------------------------------------------------------------
; Interpreter
  ...
  (lambda (exp env cont)
    (cases expression exp
      ...
      (let2-exp (var1 exp1 var2 exp2 body)
                (value-of/k exp1 env (let2-after1-cont var1 var2 exp2 body env cont)))
      )))
; ------------------------------------------------------------------------------
