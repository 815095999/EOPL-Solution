...
(define apply-procedure/k
  (lambda (proc1 vals cont saved-try-cont)
    (cases proc proc1
...
(define-datatype continuation continuation?
  ...
  (try-cont (var identifier?)
            ...
            (env environment?)
            (cont continuation?)
            (try-cont-before continuation?))  ; 之前的try需要保存
...
(define apply-cont
  (lambda (cont saved-try-cont val)
    (cases continuation cont
      ...
      (let-exp-cont (vars caled uncal body env cont)
                    (let ((new-caled (append caled (list val))))
                      (if (null? uncal)
                          (value-of/k body (extend-env-let vars new-caled env) cont saved-try-cont)
                          (value-of/k (car uncal) env (let-exp-cont vars new-caled (cdr uncal) body env cont) saved-try-cont))))
      (if-test-cont (exp1 exp2 env cont)
                    (if (expval->bool val)
                        (value-of/k exp1 env cont saved-try-cont)
                        (value-of/k exp2 env cont saved-try-cont)))
      (diff1-cont (exp2 env cont)
                  (value-of/k exp2 env (diff2-cont val cont) saved-try-cont))
      (diff2-cont (val1 cont)
                  (apply-cont cont saved-try-cont (num-val (- (expval->num val1)
                                               (expval->num val)))))
      (call-cont (caled uncal env cont)
                 (if (null? uncal)
                     (apply-procedure/k (expval->proc (car caled)) (append (cdr caled) (list val)) cont saved-try-cont)
                     (value-of/k (car uncal) env (call-cont (append caled (list val))
                                                            (cdr uncal)
                                                            env
                                                            cont)
                                                            saved-try-cont)))
      (try-cont (var exp1 env cont before-try)
                (apply-cont cont before-try val))
      (raise-cont (cont)
                  (apply-handler val saved-try-cont))
      (cons-car-cont (cdr1 env cont)
                     (value-of/k cdr1 env (cons-cdr-cont val cont) saved-try-cont))
      (cons-cdr-cont (car1 cont)
                     (apply-cont cont saved-try-cont (pair-val car1 val)))
      (car-cont (cont)
                (apply-cont cont saved-try-cont (expval->car val)))
      (cdr-cont (cont)
                (apply-cont cont saved-try-cont (expval->cdr val)))
      (null-cont (cont)
                 (apply-cont cont saved-try-cont (bool-val (cases expval val
                                              (emptylist-val () #t)
                                              (pair-val (car1 cdr1) #f)
                                              (else (eopl:error 'null "not a pair! ~s" val))))))
      (list-cont (caled uncal env cont)
                 (if (null? uncal)
                     (apply-cont cont saved-try-cont (list-reversed->expval (cons val caled)))
...
(define apply-handler
  (lambda (val cont)
    (cases continuation cont
      (try-cont (var handler-exp saved-env saved-cont before-try)
...
(define value-of/k
  (lambda (exp env cont saved-try-cont)
    (cases expression exp
      (const-exp (num) (apply-cont cont saved-try-cont (num-val num)))
      (var-exp (var)
               (apply-cont cont saved-try-cont (apply-env env var)))
      (proc-exp (vars body)
                (apply-cont cont saved-try-cont (proc-val (procedure vars body env))))
      (zero?-exp (exp1)
                 (value-of/k exp1 env (zero1-cont cont) saved-try-cont))
      (let-exp (vars exps body)
               (value-of/k (car exps) env (let-exp-cont vars '() (cdr exps) body env cont) saved-try-cont))
      (if-exp (exp1 exp2 exp3)
              (value-of/k exp1 env (if-test-cont exp2 exp3 env cont) saved-try-cont))
      (diff-exp (exp1 exp2)
        (value-of/k exp1 env (diff1-cont exp2 env cont) saved-try-cont))
      (call-exp (rator rands)
                (value-of/k rator env (call-cont '() rands env cont) saved-try-cont))
      ...
      (try-exp (exp1 var exp2)
               (value-of/k exp1 env (try-cont var exp2 env cont saved-try-cont)
                           (try-cont var exp2 env cont saved-try-cont)))
      (raise-exp (exp)
                 (value-of/k exp env (raise-cont cont) saved-try-cont))
      (cons-exp (exp1 exp2)
                (value-of/k exp1 env (cons-car-cont exp2 env cont) saved-try-cont))
      (car-exp (exp1)
      ...
      (emptylist-exp ()
                     (apply-cont cont saved-try-cont (emptylist-val)))
      (list-exp (exps)
                (if (null? exps)
                    (apply-cont cont saved-try-cont (emptylist-val))
...
letrec inner (lst) = if null?(lst) then raise 91 else if zero?(-(car(lst),n)) then 0 else -((inner cdr(lst)), -1)

in proc (lst) try (inner lst) catch (x) x in ((index 6) list(5, 5))")