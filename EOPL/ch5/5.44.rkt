...
(define-datatype continuation continuation?
  ...
  (list-cont (caled (list-of expval?))
             ...
             (env environment?)
             (cont continuation?))
  (call/cc-cont (cont continuation?))
  (return-cont (cont continuation?))
  )
      ...
      (call-cont (caled uncal env cont)
                 (if (null? uncal)
                     (cases expval (car caled)
                       ...
                       (call/cc-val (cont)
                                    (apply-cont cont val))
                       (else (eopl:error 'call-cont "rator type error!")))
      ...
      (list-cont (caled uncal env cont)
                 (if (null? uncal)
                     ...
                     (value-of/k (car uncal) env (list-cont (cons val caled)
                                                            ...
                                                            cont))))
      (call/cc-cont (cont)
                    (apply-procedure/k (expval->proc val) (list (call/cc-val cont)) (return-cont cont)))
      (return-cont (cont)
                   (apply-cont cont val))
      )))
; ------------------------------------------------------------------------------
...
(run "let y = proc (x) list((x 4),5)
         in -(3,call/cc(y))")
(run "let foo = proc (x) x
         in let bar = proc (x) 3
            in (call/cc(foo) bar)")
