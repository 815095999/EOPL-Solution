    ...
    (cases continuation cont
      (end-cont ()
                (begin (eopl:printf "End of computation.~%")
                       val))
      (zero1-cont (cont try-cont)
                  (apply-cont cont
                              (bool-val (zero? (expval->num val)))))
      (let-exp-cont (vars caled uncal body env cont try-cont)
                    (let ((new-caled (append caled (list val))))
                      (if (null? uncal)
                          (value-of/k body (extend-env-let vars new-caled env) cont)
                          (value-of/k (car uncal) env (let-exp-cont vars new-caled (cdr uncal) body env cont try-cont)))))
      (if-test-cont (exp1 exp2 env cont try-cont)
                    (if (expval->bool val)
                        (value-of/k exp1 env cont)
                        (value-of/k exp2 env cont)))
      (diff1-cont (exp2 env cont try-cont)
                  (value-of/k exp2 env (diff2-cont val cont try-cont)))
      (diff2-cont (val1 cont try-cont)
                  (apply-cont cont (num-val (- (expval->num val1)
                                               (expval->num val)))))
      (call-cont (caled uncal env cont try-cont)
                 (if (null? uncal)
      ...
      (try-cont (var exp1 env cont)
                (apply-cont cont val))
      (raise-cont (cont try-cont)
                  (apply-handler val try-cont))
      (cons-car-cont (cdr1 env cont try-cont)
                     (value-of/k cdr1 env (cons-cdr-cont val cont try-cont)))
      (cons-cdr-cont (car1 cont try-cont)
                     (apply-cont cont (pair-val car1 val)))
      (car-cont (cont try-cont)
                (apply-cont cont (expval->car val)))
      (cdr-cont (cont try-cont)
                (apply-cont cont (expval->cdr val)))
      (null-cont (cont try-cont)
                 (apply-cont cont (bool-val (cases expval val
                                              (emptylist-val () #t)
                                              (pair-val (car1 cdr1) #f)
                                              (else (eopl:error 'null "not a pair! ~s" val))))))
      (list-cont (caled uncal env cont try-cont)
                 (if (null? uncal)
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      ...
      (zero?-exp (exp1)
                 (value-of/k exp1 env (zero1-cont cont (try-cont-of cont))))
      (let-exp (vars exps body)
               (value-of/k (car exps) env (let-exp-cont vars '() (cdr exps) body env cont (try-cont-of cont))))
      (if-exp (exp1 exp2 exp3)
              (value-of/k exp1 env (if-test-cont exp2 exp3 env cont (try-cont-of cont))))
      (diff-exp (exp1 exp2)
        (value-of/k exp1 env (diff1-cont exp2 env cont (try-cont-of cont))))
      (call-exp (rator rands)
                (value-of/k rator env (call-cont '() rands env cont (try-cont-of cont))))
      ...
      (raise-exp (exp)
                 (value-of/k exp env (raise-cont cont (try-cont-of cont))))
      (cons-exp (exp1 exp2)
                (value-of/k exp1 env (cons-car-cont exp2 env cont (try-cont-of cont))))
      (car-exp (exp1)
               (value-of/k exp1 env (car-cont cont (try-cont-of cont))))
      (cdr-exp (exp1)
               (value-of/k exp1 env (cdr-cont cont (try-cont-of cont))))
      (null-exp (exp1)
                (value-of/k exp1 env (null-cont cont (try-cont-of cont))))
      (emptylist-exp ()
