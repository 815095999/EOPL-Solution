...
; 求deep是O(n)的，因此递归复杂度为O(n^2)，可以简化为O(1) 求deep，但是需要在所有构造continuation的地方加以改动。
(define deep
  (lambda (cont)
    (cases continuation cont
      (end-cont () 0)
      (zero1-cont (cont) (+ 1 (deep cont)))
      (let-exp-cont (vars caled uncal body env cont)
                    (+ 1 (deep cont)))
      (if-test-cont (exp1 exp2 env cont) (+ 1 (deep cont)))
      (diff1-cont (exp2 env cont) (+ 1 (deep cont)))
      (diff2-cont (val1 cont) (+ 1 (deep cont)))
      (mul1-cont (exp2 env cont) (+ 1 (deep cont)))
      (mul2-cont (val1 cont) (+ 1 (deep cont)))
      (rator-cont (rand env cont) (+ 1 (deep cont)))
      (rand-cont (rator cont) (+ 1 (deep cont)))
      (call-cont (caled uncal env cont) (+ 1 (deep cont)))
      (begin-cont (exps env cont) (+ 1 (deep cont))))))
...
(define value-of-program
  (lambda (pgm)
    (set! max-depth 0)
    (let ((finalvalue (cases program pgm
                        (a-program (exp1)
                                   (value-of/k exp1 (init-env) (end-cont))))))
      (begin (display "max-continuation-depth ")
             (display max-depth)
             (newline)
             finalvalue))))
