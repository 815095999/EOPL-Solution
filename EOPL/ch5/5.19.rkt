  ...
  (lambda (cont val)
    (cases continuation cont
      ...
      (diff2-cont (val1 cont)
                  (lambda () (apply-cont cont (num-val (- (expval->num val1)
                                                          (expval->num val))))))
...
(define value-of/k
  (lambda (exp env cont)
    (cases expression exp
      (const-exp (num) (lambda () (apply-cont cont (num-val num))))
      (var-exp (var)
               (lambda () (apply-cont cont (apply-env env var))))
      (proc-exp (var body)
                (lambda () (apply-cont cont (proc-val (procedure var body env)))))
      (zero?-exp (exp1)
