    ...
    (cases continuation cont
      ...
      (diff2-cont (val1 saved-cont)
                  (let ((num1 (expval->num val1))
                    ...
                    (apply-cont)))
      (call-cont (caled uncal env saved-cont)
                 (if (null? uncal)
                     (begin
                       (set! cont saved-cont)
                       (set! val (append (cdr caled) (list val)))
                       (set! proc1 (expval->proc (car caled)))
                       (apply-procedure/k))
                     (begin
                       (set! cont (call-cont (append caled (list val))
                                             (cdr uncal)
                                             env
                                             saved-cont))
                       (set! exp (car uncal))
...
(run "letrec sumiter(x,y) = if zero?(x) then y else (sumiter -(x,1) -(y,-(0,x))) in (sumiter 100 0)")