...
(define refto-type->type
  (lambda (ty)
    (cases type ty
      (refto-type (ty1) ty1)
      (else (eopl:error 'refto-type->type "not a refto-type ~s" ty)))))
...
(define apply-one-subst
  (lambda (ty0 tvar ty1)
    (cases type ty0
      ...
      (refto-type (t1)
                  (refto-type (apply-one-subst t1 tvar ty1)))
      (tvar-type (sn)
...
(define apply-subst-to-type
  (lambda (ty subst)
    (cases type ty
      ...
      (refto-type (t1)
                  (refto-type (apply-subst-to-type t1 subst)))
      (tvar-type (sn)
  ...
  (lambda (exp tenv subst)
    (cases expression exp
      ...
      (newref-exp (exp1)
                  (cases answer (type-of exp1 tenv subst)
                    (an-answer (exp1-type subst)
                               (an-answer (refto-type exp1-type) subst))))
      (deref-exp (exp1)
                 (cases answer (type-of exp1 tenv subst)
                   (an-answer (exp1-type subst)
                              (an-answer (refto-type->type exp1-type) subst))))
                    ...
                    (an-answer (exp1-type subst)
                               (cases answer (type-of exp2 tenv subst)
                                 (an-answer (exp2-type subst)
                                            (an-answer (int-type)
                                                       (unifier exp1-type
                                                                (refto-type exp2-type)
                                                                subst exp)))))))
      )))


 (run "let x = newref (3) in let y = newref(x) in proc(z:?) proc (w:?) newref(setref(w,if zero?(0) then z else deref(y)))")
