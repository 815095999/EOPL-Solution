...
(define run
  (lambda (string)
    (type-to-external-for(type-of-program (scan&parse string)))))
    ...
    (cases expression exp
      ...
      (list-exp (exp1 exps)
                (let ((ty1 (type-of exp1 tenv)))
                  (if (null? exps)
                      (list-type ty1)
                      (let ((ty2 (type-of (list-exp (car exps) (cdr exps)) tenv)))
                        (cases type ty2
                          (list-type (ty3)
                                     (check-equal-type! ty1 ty3 exp)
                                     ty2)
                          (else (eopl:error 'list-exp "~s is not a list: ~s" exp ty2)))))))
      (cons-exp (exp1 exp2)
                (let ((ty1 (type-of exp1 tenv))
                      (ty2 (type-of exp2 tenv)))
                  (cases type ty2
                    (list-type (ty3)
                               (check-equal-type! ty1 ty3 exp)
                               ty2)
                    (else (eopl:error 'cons-exp "~s is not a list: ~s" exp2 ty2)))))
      (null-exp (exp1)
                (let ((ty1 (type-of exp1 tenv)))
                  (cases type ty1
                    (list-type (ty2)
                               (bool-type))
                    (else (eopl:error 'null-exp "~s is not a list: ~s" exp1 ty1)))))
      (emptylist-exp (ty1)
                     (list-type ty1))
      (car-exp (exp1)
               (let ((ty1 (type-of exp1 tenv)))
                 (cases type ty1
                   (list-type (ty2)
                              ty2)
                   (else (eopl:error 'car-exp "~s is not a list: ~s" exp1 ty1)))))
      (cdr-exp (exp1)
               (let ((ty1 (type-of exp1 tenv)))
                 (cases type ty1
                   (list-type (ty2)
                              ty1)
                   (else (eopl:error 'cdr-exp "~s is not a list: ~s" exp1 ty1)))))
      )))

(run "cons(3, list(1,3,4,6))")