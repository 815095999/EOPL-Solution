      ...
      (deref-exp (exp1)
                 (let ((ty1 (type-of exp1 tenv)))
                   (cases type ty1
                     (refto-type (ty2)
                                 ty2)
                     (else (eopl:error 'deref-exp "~s not a refto type : ~s" exp1 ty1)))))
                  ...
                  (let ((ty1 (type-of exp1 tenv))
                    ...
                    (cases type ty1
                      (refto-type (ty3)
                                  (check-equal-type! ty2 ty3 exp)
                                  (void-type))
                      (else (eopl:error 'setref-exp "~s not a refto type : ~s" exp1 ty1)))))
      )))


(run "let x = newref(3) in deref(newref(newref(x)))")