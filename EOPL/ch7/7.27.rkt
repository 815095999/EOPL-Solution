    ...
    (cases program pgm
      (a-program (exp1)
                 (cases answer (type-of exp1 (init-tenv) (empty-subst))
                   (an-answer (ty eqs)
                              (apply-subst-to-type ty (solve-eqs eqs))))))))
(define run
  (lambda (str)
...
(define type-of
  (lambda (exp tenv eqs)
    (cases expression exp
      (const-exp (num) (an-answer (int-type) eqs))
      (zero?-exp (exp1)
                 (cases answer (type-of exp1 tenv eqs)
                   (an-answer (ty1 eqs)
                              (let ((eqs
                                     (cons (cons ty1 (int-type)) eqs)))
                                (an-answer (bool-type) eqs)))))
      (diff-exp (exp1 exp2)
                (cases answer (type-of exp1 tenv eqs)
                  (an-answer (ty1 eqs)
                             (let ((eqs
                                    (cons (cons ty1 (int-type)) eqs)))
                               (cases answer (type-of exp2 tenv eqs)
                                 (an-answer (ty2 eqs)
                                            (let ((eqs
                                                   (cons (cons ty2 (int-type)) eqs)))
                                              (an-answer (int-type) eqs))))))))
      (if-exp (exp1 exp2 exp3)
              (cases answer (type-of exp1 tenv eqs)
                (an-answer (ty1 eqs)
                           (let ((eqs
                                  (cons (cons ty1 (bool-type)) eqs)))
                             (cases answer (type-of exp2 tenv eqs)
                               (an-answer (ty2 eqs)
                                          (cases answer (type-of exp3 tenv eqs)
                                            (an-answer (ty3 eqs)
                                                       (let ((eqs
                                                              (cons (cons ty2 ty3) eqs)))
                                                         (an-answer ty2 eqs))))))))))
      (var-exp (var)
      ...
      (call-exp (rator rand)
                (let ((result-type (fresh-tvar-type)))
                  (cases answer (type-of rator tenv eqs)
                    (an-answer (rator-type eqs)
                               (cases answer (type-of rand tenv eqs)
                                 (an-answer (rand-type eqs)
                                            (let ((eqs
                                                   (cons (cons rator-type
                                                               (proc-type rand-type result-type))
                                                         eqs)))
                                              (an-answer result-type eqs))))))))
                  ...
                  (let ((p-result-type (otype->type p-result-otype))
                    ...
                    (let ((tenv-for-letrec-body (extend-tenv p-name
                      ...
                      (cases answer (type-of p-body (extend-tenv b-var p-var-type
                                             ...
                                             eqs)
                        (an-answer (p-body-type eqs)
                                   (let ((eqs
                                          (cons (cons p-body-type p-result-type) eqs)))
                                     (type-of letrec-body
                                              tenv-for-letrec-body
                                              eqs)))))))
      )))
