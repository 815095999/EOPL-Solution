...
(define grammar
  '((program ((arbno module-definition)
              "import" (separated-list identifier ",")
              "[" expression "]") a-program)
    (module-definition ("module" identifier "interface" interface "body" "import"
                                 (separated-list identifier ",") module-body) a-module-definition)
...
(define-datatype environment environment?
  ...
  [extend-env-with-module [m-name symbol?]
                          [m-val typed-module?]
                          [saved-env environment?]]
  [extend-env-with-module-delayed [m-name symbol?]
                                  [m-val procedure?]
                                  [saved-env environment?]])
    ...
    (cases environment env
      ...
      [extend-env-with-module (m-name m-val saved-env)
                              (if (eqv? search-sym m-name)
                                  m-val
                                  (apply-env saved-env search-sym))]
      [extend-env-with-module-delayed (m-name m-val saved-env)
                                      (if (eqv? search-sym m-name)
                                          m-val
                                          (apply-env saved-env search-sym))])))
...
(define add-depends-to-tenv
  (lambda (depends tenv src-tenv)
    (if (null? depends)
        tenv
        (add-depends-to-tenv (cdr depends)
                             (extend-tenv-with-module
                              (car depends)
                              (apply-tenv src-tenv (car depends))
                              tenv)
                             src-tenv))))
(define type-of-program
  (lambda (pgm)
    (cases program pgm
      (a-program (module-defns depends body)
                 (let ((module-defns-tenv (add-module-defns-to-tenv module-defns (empty-tenv))))
                   (type-of body
                            (add-depends-to-tenv depends
                                                 (empty-tenv)
                                                 module-defns-tenv)))))))
  ...
  (lambda (defns tenv)
    (if (null? defns)
        ...
        (cases module-definition (car defns)
          (a-module-definition (m-name expected-iface depends m-body)
                               (let ((actual-iface (interface-of m-body
                                                                 (add-depends-to-tenv depends
                                                                                      (empty-tenv)
                                                                                      tenv))))
...
(define type-of
  (lambda (exp tenv)
    (cases expression exp
      ...
      (print-exp (exp1) (begin (type-of exp1 tenv)
                               (int-type)))
      )))


; ------------------------------------------------------------------------------
(define add-depends-to-env
  (lambda (depends env src-env)
    (if (null? depends)
        env
        (add-depends-to-env
         (cdr depends)
...
(define value-of-program
  (lambda (pgm)
    (cases program pgm
      (a-program (m-defns depends body)
                 (let ((module-defns-env (add-module-defns-to-env m-defns (empty-env))))
                   (value-of body
                             (add-depends-to-env depends
                                                 (empty-env)
                                                 module-defns-env)))))))
...
(define add-module-defns-to-env
  (lambda (defns env)
    (if (null? defns)
        env
        (cases module-definition (car defns)
               (a-module-definition (m-name iface depends m-body)
                                    (add-module-defns-to-env
                                     (cdr defns)
                                     (extend-env-with-module-delayed
                                      m-name
                                      (lambda ()
                                        (value-of-module-body m-body (add-depends-to-env
                                                                    depends
                                                                    (empty-env)
                                                                    env)))
                                      env)))))))
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (print-exp (exp1)
                 (begin (display (value-of exp1 env))
                        (num-val 1)))
      )))
...
(run-val "module m1
           interface [x : int]
           body import [x = print(1)]
         module m2
           interface []
           body import [x = print(2)]
         module m3
           interface []
           body import m2 [x = print(3)]
        import m3, m1 [-(from m1 take x, 1)]")