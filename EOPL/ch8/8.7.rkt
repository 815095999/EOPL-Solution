...
(define lookup-qualified-var-in-mod
  (lambda (mod1 var-name)
    (cases typed-module mod1
      (simple-module (bindings)
                     (apply-env bindings var-name)))))
; lookup-qualified-var-in-env : sym x sym x env -> expval
(define lookup-qualified-vars-in-mod
  (lambda (mod1 var-names)
    (let ((fir (lookup-qualified-var-in-mod mod1 (car var-names))))
      (if (null? (cdr var-names))
          fir
          (lookup-qualified-vars-in-mod (expval->module fir) (cdr var-names))))))

; lookup-qualified-var-in-tenv : sym x sym x tenv -> type
(define lookup-qualified-var-in-modtype
  (lambda (mod1 var-name)
    (cases type mod1
      (module-type (iface)
      ...
      (else (eopl:error lookup-qualified-var-in-modtype "not a interface ~s" mod1)))))
(define lookup-qualified-vars-in-modtype
  (lambda (mod1 var-names)
    (let ((fir (lookup-qualified-var-in-modtype mod1 (car var-names))))
      (if (null? (cdr var-names))
          fir
          (lookup-qualified-vars-in-modtype fir (cdr var-names))))))
...
(define type-of
  (lambda (exp tenv)
    (cases expression exp
      ...
      (qualified-var-exp (m-var vars)
                         (lookup-qualified-vars-in-modtype (apply-tenv tenv m-var)
                                                           vars))
      )))
  ...
  (lambda (mod1 env)
    (cases module-definition mod1
      (a-module-definition (m-name iface m-body)
                           (extend-env
                            m-name
                            (module-val (value-of-module-body m-body env))
                            env)))))
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (qualified-var-exp (m-name vars)
                         (lookup-qualified-vars-in-mod (expval->module (apply-env env m-name))
                                                       vars))
      )))
