...
(define grammar
  '((program ((arbno module-definition) expression) a-program)
    ...
    (module-body ("module-proc" "(" (arbno identifier ":" interface) ")" module-body) proc-module-body)
...
(define expand-iface
  (lambda (m-name iface tenv)
    (cases interface iface
      ...
      (proc-iface (param-names param-ifaces result-iface) iface))))
...
(define interface-of
  (lambda (m-body tenv)
    (cases module-body m-body
      ...
      (app-module-body (rator-id rand-ids)
                       (let ((rator-iface (lookup-module-name-in-tenv tenv rator-id))
                             (rand-ifaces (map (lambda (x) (lookup-module-name-in-tenv tenv x)) rand-ids)))
                         (cases interface rator-iface
                           ...
                           (proc-iface (param-names param-ifaces result-iface)
                                       (if (<:-ifaces rand-ifaces param-ifaces tenv)
                                           (let loop ([result-iface result-iface]
                                                      [param-names param-names]
                                                      [rand-ids rand-ids])
                                             (if (null? param-names)
                                                 result-iface
                                                 (loop (rename-in-iface result-iface
                                                                        (car param-names)
                                                                        (car rand-ids))
                                                       (cdr param-names)
                                                       (cdr rand-ids))))
                                           (report-bad-module-application-error param-ifaces rand-ifaces m-body))))))
      (proc-module-body (rand-names rand-ifaces m-body)
                        (let ((body-iface (interface-of m-body (extend-tenv-with-module* rand-names
                                                                                         rand-ifaces
                                                                                         tenv))))
                          (proc-iface rand-names rand-ifaces body-iface))))))
(define extend-tenv-with-module*
  (lambda (rand-names rand-ifaces tenv)
    (if (null? rand-names)
        tenv
        (extend-tenv-with-module* (cdr rand-names)
                                  (cdr rand-ifaces)
                                  (extend-tenv-with-module (car rand-names)
                                                           (expand-iface (car rand-names)
                                                                         (car rand-ifaces) tenv)
                                                           tenv)))))
(define <:-ifaces
  (lambda (rand-ifaces param-ifaces tenv)
    (if (null? param-ifaces)
        #t
        (and (<:-iface (car rand-ifaces)
                       (car param-ifaces)
                       tenv)
             (<:-ifaces (cdr rand-ifaces)
                        (cdr param-ifaces)
                        tenv)))))
...
(define rename-in-iface
  (lambda (iface ori-name now-name)
    (cases interface iface
      ...
      (proc-iface (param-names param-ifaces result-iface)
                  (proc-iface (map (lambda (param-name) (if (equal? param-name ori-name) now-name param-name))
                                   param-names)
                              (map (lambda (param-iface) (rename-in-iface param-iface ori-name now-name))
                                   param-ifaces)
    ...
    (cases interface iface1
      (simple-iface (decls1)
                    (cases interface iface2
                      ...
                      (else #f)))
      (proc-iface (param-names1 param-ifaces1 result-iface1)
                  (cases interface iface2
                    (simple-iface (decls2) #f)
                    (proc-iface (param-names2 param-ifaces2 result-iface2)
                                (let ((new-names (map (lambda (x) (fresh-module-name x)) param-names1)))
                                  (let ((result-iface1
                                         (rename-in-iface-names result-iface1 param-names1 new-names))
                                        (result-iface2
                                         (rename-in-iface-names result-iface2 param-names2 new-names)))
                                    (and (<:-ifaces param-ifaces2 param-ifaces1 tenv)
                                         (<:-iface result-iface1 result-iface2
                                                   (extend-tenv-with-module*
                                                    new-names
                                                    param-ifaces1
                                                    tenv)))))))))))
(define rename-in-iface-names
  (lambda (iface ori-names new-names)
    (if (null? new-names)
        iface
        (rename-in-iface-names (rename-in-iface iface (car ori-names) (car new-names))
                               (cdr ori-names)
                               (cdr new-names)))))
...
(define value-of-module-body
  (lambda (m-body env)
    (cases module-body m-body
      ...
      (proc-module-body (m-names m-types m-body)
                        (proc-module m-names m-body env))
      (app-module-body (rator rands)
                       (let ((rator-val (lookup-module-name-in-env env rator))
                             (rand-vals (map (lambda (rand) (lookup-module-name-in-env env rand)) rands)))
                         (cases typed-module rator-val
                           (proc-module (m-names m-body env)
                                        (value-of-module-body m-body
                                                              (extend-env-with-module* m-names rand-vals env)))
                           (else (report-bad-module-app rator-val))))))))
(define extend-env-with-module*
  (lambda (m-names rand-vals env)
    (if (null? m-names)
        env
        (extend-env-with-module* (cdr m-names)
                                 (cdr rand-vals)
                                 (extend-env-with-module (car m-names)
                                                         (car rand-vals)
                                                         env)))))
