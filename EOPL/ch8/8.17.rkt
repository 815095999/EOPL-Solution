  ...
  (lambda (decls1 decls2 tenv)
    (if (null? decls2) #t
        (let ((name2 (decl->name (car decls2))))
          (letrec ((loop (lambda (name decls)
                          (if (null? decls)
                              #f
                              (if (equal? name (decl->name (car decls)))
                                  (car decls)
                                  (loop name (cdr decls)))))))
            (let ((decl1 (loop name2 decls1)))
              (if decl1
                  (and (<:-decl decl1 (car decls2) tenv)
                       (<:-decls decls1 (cdr decls2)
                                (extend-tenv-with-decl decl1 tenv)))
                  #f)))))))
...
(run-val
 "module tables
   interface [opaque table
              empty : table
              add-to-table : (int -> (bool -> (table -> table)))
              lookup-in-table : (int -> (table -> bool))]
   body
     [type table = (int -> bool)
      empty = proc (x : int) zero?(1)
      add-to-table = proc (x : int) proc (y : bool) proc (tab : table)
   ...
   in let add-binding = from tables take add-to-table
      in let lookup = from tables take lookup-in-table
         in let table1 = (((add-binding 3) zero?(1)) (((add-binding 4) zero?(1)) (((add-binding 3) zero?(1)) empty)))
            in if ((lookup 4) table1) then 1 else 2")

(run-val "module m interface [opaque t true : t false : t] body [true = 1 false = 0 type t = int] 1")