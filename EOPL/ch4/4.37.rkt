...
(define-datatype mutpair mutpair?
  (a-pair (fir reference?)))
; make-pair: expval x expval -> mutpair
(define make-pair
  (lambda (val1 val2)
    (let ((add1 (newref val1))
          (add2 (newref val2)))
      (a-pair add1))))
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (call-exp (rator rands)
                (let* ((old-and-news (map (lambda (rand) (old-and-new rand env)) rands))
                       (olds (map car old-and-news))
                       (news (map cdr old-and-news))
                       (val (apply-procedure (expval->proc (value-of rator env)) news)))
                  (begin
                    (cover olds news)
                    val)))
      (zero?-exp (exp1)
...
(define old-and-new
  (lambda (exp env)
    (cases expression exp
      (var-exp (var)
               (let* ((add1 (apply-env env var))
                      (add2 (newref (deref add1))))
                 (cons add1 add2)))
      (else (eopl:error 'call-exp "argument can only be variable! ~s" exp)))))
(define cover
  (lambda (olds news)
    (if (null? news)
        'ok
        (begin (setref! (car olds) (deref (car news)))
               (cover (cdr olds) (cdr news))))))
(define begin-val
  (lambda (exps env)
    (if (= (length exps) 1)
...
set x = 4 in begin (p b); b end") ;答案一致

(run "let b = 3
  in let p = proc (x) proc(y) begin
      set x = 4;
        y end in begin ((p b) b)
               ; b end")    ;答案不一致，因为第一次传入的x返回procedure并没有改变原location，第二次传入y改变的却是第一次的x，也不会影响原location