...
; call-by-reference: 精髓在于procedure中复制var的地址，想改变数组则还需复制数组的地址
; ref表达式精髓在于expval中新增ref-val，用deref得到ref-val中ref所存储的值
...
(define-datatype array array?
  (a-array (first-loc reference?)
           (len integer?)))

; arrayref:  array x number -> expval
(define arrayref
  (lambda (a p)
    (cases array a
      (a-array (fir len)
               (if (> p (- len 1))
                   (report-array-out-of-range 'arrayref a p)
                   (deref (+ fir p)))))))
; arrayset: array x number x expval -> unspecified
(define arrayset
  (lambda (a p v)
    (cases array a
      (a-array (fir len)
               (if (> p (- len 1))
                   (report-array-out-of-range 'arrayset a p)
                   (setref! (+ fir p) v))))))

; make-array: listof(expression) x env -> array
(define make-array
  (lambda (exps env)
    (if (null? exps)
        (a-array -1 0)
        (let ((fir (newref (value-of (car exps) env)))
              (rest (make-array (cdr exps) env)))
          (a-array fir (length exps))))))
(define report-array-out-of-range
  (lambda (info arr l)
    (eopl:error info "out of range")))
    ...
    (cases expression exp
      ...
      (call-exp (rator rands)
                (apply-procedure (expval->proc (value-of rator env))
                                 (map (lambda (rand) (value-of-operand rand env)) rands)))
      (zero?-exp (exp1)
                 (let ((val1 (value-of exp1 env)))
                   (let ((num1 (expval->num val1)))
                     (bool-val (zero? num1)))))
      ...
      (newarray-exp (exps)
                    (array-val (make-array exps env)))
      (arrayref-exp (exp1 pos)
                    (arrayref (expval->array (value-of exp1 env))
                               (expval->num (value-of pos env))))
      (arrayset-exp (exp1 pos exp2)
                    (arrayset (expval->array (value-of exp1 env))
                               (expval->num (value-of pos env))
                               (value-of exp2 env)))
      (arraylength-exp (exp1)
                       (cases array (expval->array (value-of exp1 env))
                         (a-array (fir len)
                                  (num-val len))))
      )))
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (arrayref-exp (exp1 pos)
                     (cases array (expval->array (value-of exp1 env))
                       (a-array (fir len)
                                (+ fir (expval->num (value-of pos env))))))
...
(define read-eval-print
 ...
 (run "let swap = proc(x) proc (y) let temp = x
                                    in begin
                                          set x = y;
                                          set y = temp
                                       end
       in let a = newarray(1,0)
          in begin
               ((swap arrayref(a,0)) arrayref(a,1));
               -(arrayref(a,arrayref(a,0)),arrayref(a,1))
             end")