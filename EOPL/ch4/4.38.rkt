...
; 之前的代码均可兼容
; 当前代码会在call-by-value时死循环，因为(d d)产生无限循环，但是现在(d d)传入f时只是一个thunk，要在进一步传入maketimes4的第二个参数时才可能会用到，因此不会循环
; ------------------------------------------------------------------------------
; Storage
...
(run "let makerec = proc (f) let d = proc (x) proc (z) ((f (x x)) z) in proc (n) ((f (d d)) n) in let maketimes4 = proc (f) proc (x) if zero?(x) then 0 else -((f -(x,1)), -4)

in let times4 = (makerec maketimes4)

in (times4 3)")

(run "let makerec = proc (f) let d = proc (x) (f (x x)) in (f (d d)) in let maketimes4 = proc (f) proc (x) if zero?(x) then 0 else -((f -(x,1)), -4)
