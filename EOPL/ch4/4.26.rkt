  ...
  (lambda (sta env)
    (cases statement sta
      ...
      (var-sta (var1 exp1 vars exps sta)
               (let* ((p-names (cons var1 vars))
                      (lvars-and-bodys (extract-lvars-and-bodys (cons exp1 exps)))
                      (lvars (car lvars-and-bodys))
                      (bodys (cdr lvars-and-bodys)))
                 (result-of sta (extend-env-rec* p-names lvars bodys env))))
      (read-sta (var1)
                (let ((val (read)))
                  (if (and (integer? val)
                           (> val -1))
...
(define extract-lvars-and-bodys
  (lambda (exps)
    (if (null? exps)
        (cons '() '())
        (cases expression (car exps)
          (proc-exp (vars body)
                    (let ((temp (extract-lvars-and-bodys (cdr exps))))
                      (cons (cons vars (car temp))
                            (cons body (cdr temp)))))
          (else (eopl:error 'extract-lvars-and-bodys "not a proc ~s" (car exps)))))))
; ------------------------------------------------------------------------------
...
(run "var odd = proc (x) if zero?(x) then 0 else (even -(x,1)),even = proc(x) if zero?(x) then 1 else (odd -(x,1)); {print (even 17)}")