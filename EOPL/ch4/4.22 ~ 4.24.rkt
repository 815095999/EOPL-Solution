...
(define grammar
  '((program (statement) a-program)
    ...
    (statement ("var" identifier (arbno "," identifier) ";" statement) var-sta)
...
(define result-of
  (lambda (sta env)
    (cases statement sta
      ...
      (var-sta (var1 vars sta)
               (result-of sta (extend-env-vars (cons var1 vars) env)))
      (read-sta (var1)
                (let ((val (read)))
                  (if (and (integer? val)
                           (> val -1))
  ...
  (lambda (vars env)
    (if (null? vars)
        ...
        (extend-env-vars (cdr vars)
                         (extend-env (car vars) (newref 'uninitialized) env)))))
; ------------------------------------------------------------------------------
