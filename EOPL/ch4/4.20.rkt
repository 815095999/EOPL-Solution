    ...
    (define (modify-vec vecs b-vars bodys env num)
      (if (null? b-vars)
          ...
          (begin (vector-set! (list-ref vecs num) 0
                              (cons #f (newref (proc-val (procedure (car b-vars) (car bodys) env)))))
...
(define apply-env
  (lambda (env search-var)
    (cases environment env
      ...
      (extend-env (saved-var saved-mutable saved-val saved-env)
                  (if (equal? saved-var search-var)
                      (cons saved-mutable saved-val)
...
(define init-env
  (lambda ()
    (extend-env 'i #t (newref (num-val 1))
      (extend-env 'v #t (newref (num-val 5))
        (extend-env 'x #t (newref (num-val 10))
          (empty-env))))))
; ------------------------------------------------------------------------------
    ...
    (cond ((= (length vars) (length vals))
           (cond ((null? vars) env)
                 (else (extend-env* (cdr vars)
                                    (cdr vals)
                                    (extend-env (car vars) #t (newref (car vals)) env)))))
...
(define value-of
  (lambda (exp env)
    (cases expression exp
      ...
      (if-exp (exp1 exp2 exp3)
        (let ((val1 (value-of exp1 env)))
          (if (expval->bool val1)
              ...
              (value-of exp3 env))))
      (var-exp (var)
        (deref (cdr (apply-env env var))))
      (let-exp (vars exps body)
               (let ((env1 (extend-env-let vars exps env env #f)))
                 (value-of body env1)))
      (proc-exp (vars body)
      ...
      (assign-exp (var exp)
                  (let ((con (apply-env env var)))
                    (if (car con)
                        (begin (setref! (cdr con)
                                        (value-of exp env))
                               (num-val 27))
                        (eopl:error 'assign "can't modify immutable variable ~a" var))))
      (begin-exp (fir-exp rest-exps)
                 (begin-val (cons fir-exp rest-exps) env))
      (letmutable-exp (vars exps body)
               (let ((env1 (extend-env-let vars exps env env #t)))
                 (value-of body env1)))
      )))
(define begin-val
  (lambda (exps env)
...
(define extend-env-let
  (lambda (vars exps env ori_env flag)
    (if (null? vars)
        env
...
(run "let x = 4 in begin
               set x = 5;
               -(1, x) end")
(run "letmutable x = 4 in
        begin
        set x = 5;
        -(1, x) end")