...
(run "let fact = proc (n) +(n,1) in let fact = proc (n) if zero?(n) then 1 else * (n,(fact -(n,1))) in (fact 5)")
(run "let even? = proc (n) if zero?(n) then zero?(0) else (odd? -(n,1))
         in let odd? = proc (n) if zero?(n) then zero?(1) else (even? -(n,1))
             in (even? 11)")
