...
; 同3.23,主要想法是多传入一个continuation,为了统一把bool-exp全改为expression,互相调用则还需额外增加一个参数
; ------------------------------------------------------------------------------
; Environments
...
(display (run "let odd? = proc (n, cont1, cont2) if zero?(n) then zero?(1) else (cont1 -(n,1) cont2 cont1)
                   in let even? = proc (n, cont1, cont2) if zero?(n) then zero?(0) else (cont1 -(n,1) cont2 cont1)
                     in let even2? = proc (n) (even? n odd? even?)
                       in (even2? 101)"))