#lang eopl
; 为了不用区分+-*/,zero?等，只需要改一改grammar的匹配方式即可。
; ------------------------------------------------------------------------------
; Environments
...
(define extend-env*
  (lambda (vars vals env)
    (cond ((= (length vars) (length vals))
          ...
          (else (eopl:error "not enough arguments! ~% expected: ~s detected: ~s" (length vars) (length vals))))))
...
(run "let f = (proc (x y z) (* x (+ y z))) in let g = (proc (x y z) (* (f x y z) (f x y z))) in (g 4 6 4)")
