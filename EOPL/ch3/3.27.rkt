...
(define expval->trproc
  (lambda (val)
    (cases expval val
      (trproc-val (proc1) proc1)
      (else (report-expval-extractor-error 'trproc-val val)))))
...
(define-datatype proc proc?
  (procedure (vars (list-of identifier?))
             (body expression?)
             (saved-env environment?)))
(define-datatype trproc trproc?
  (trprocedure (vars (list-of identifier?))
               (body expression?)
               (saved-env environment?)))
...
(define apply-tr-procedure
  (lambda (proc1 vals)
    (cases trproc proc1
      (trprocedure (vars body saved-env)
                   (display "enter a traceproc")
                   (newline)
                   (let ((val (value-of body (extend-env* vars vals saved-env))))
                     (display "exit a traceproc")
                     (newline)
                     val)))))
  ...
  (lambda (exp env)
    (cases expression exp
      ...
      (traceproc-exp (vars body)
                     (trproc-val (trprocedure vars body env)))
      (call-exp (rator rands)
                (cases expval (value-of rator env)
                  (proc-val (proc1) (apply-procedure proc1
                                                     (map (lambda (rand) (value-of rand env)) rands)))
                  (trproc-val (proc1) (apply-tr-procedure proc1
                                                          (map (lambda (rand) (value-of rand env)) rands)))
                  (else (eopl:error "application not a proc/trproc "))))
      (zero?-exp (exp1)
                 (let ((val1 (value-of exp1 env)))
                   (let ((num1 (expval->num val1)))
                     (bool-val (zero? num1)))))
...
(display (run "let makerec = traceproc (f) let d = traceproc (x) traceproc (z) ((f (x x)) z) in proc (n) ((f (d d)) n) in let maketimes4 = proc (f) proc (x) if zero?(x) then 0 else -((f -(x,1)), -4)
